...
```bash
# macbook pro
sudo curl -L /tmp https://github.com/team-xquare/xctl/releases/download/v0.2.5/xctl-v0.2.5-darwin-amd64.tar.gz > /tmp/xctl.tar.gz

# macbook air
sudo curl -L /tmp https://github.com/team-xquare/xctl/releases/download/v0.2.5/xctl-v0.2.5-darwin-arm64.tar.gz > /tmp/xctl.tar.gz
```
...
```bash
sudo mv /tmp/xctl-v0.2.5-darwin-amd64 /usr/local/bin/xctl

or

sudo mv /tmp/xctl-v0.2.5-darwin-arm64 /usr/local/bin/xctl
...
```bash
sudo curl -L /tmp https://github.com/team-xquare/xctl/releases/download/v0.2.5/xctl-v0.2.5-linux-amd64.tar.gz > /tmp/xctl.tar.gz
```
...
```bash
sudo mv /tmp/xctl-v0.2.5-linux-amd64 /usr/local/bin/xctl
mkdir $HOME/.xctl
sudo mv /tmp/template $HOME/.xctl/
# You can get github token from the XQUARE notion page.
xctl set credential -t <xquare-admin_github_token>
